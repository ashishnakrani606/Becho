        ...
        <div>
          <div className="relative">
            <Listbox.Button className="relative w-full cursor-pointer bg-white dark:bg-transparent text-left focus:border-0 focus:outline-none focus:ring-0 sm:text-sm">
              <span className={`truncate text-black/20 dark:text-white/20 ${props.className}`}>{selected.name}</span>
            ...
            <Transition
              ...
              leaveTo="opacity-0"
            >
              <Listbox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white dark:bg-black text-base shadow-lg ring-1 ring-black dark:ring-black dark:border-white/10 dark:border ring-opacity-5 focus:outline-none sm:text-sm">
                {props.selectitem.map((item) => (
                  <Listbox.Option
                    key={item.id}
                    className={({ active }) =>
                      classNames(
                        active ? 'text-white bg-black dark:bg-white dark:text-black' : 'text-black/80 dark:text-white/80',
