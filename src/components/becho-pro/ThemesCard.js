...
const ThemesCard = ({
  ...
  return (
    <div
      className={
        "w-full bg-primary-light dark:bg-white/5 rounded-2xl md:p-6 p-4"
      }
    >
      <span
      ...
      <h4 className="text-blacklight dark:text-white mb-1 font-semibold">
        {title}
      </h4>
      <p className="text-black/40 dark:text-white/40 text-xs">{description}</p>
      <div className="flex items-center mt-2 justify-between">
        <Link
          className="flex items-center text-black/40 dark:text-white/40 text-xs"
          href={"/"}
        >
